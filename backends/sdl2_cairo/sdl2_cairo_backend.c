...
result_text_dimensions sdl2_cairo_backend_get_text_dimensions(const char* text, const char* font_name, uint8 font_size);
...
result_render_backend_ptr sdl2_cairo_backend_create() {
  ...
  if (!backend) {
    error(result_render_backend_ptr, "Unable to allocate memory for SDL2+Cairo render backend!");
  }
...
result_text_dimensions sdl2_cairo_backend_get_text_dimensions(const char* text, const char* font_name, uint8 font_size) {
  if (!text) {
    return error(result_text_dimensions, "Cannot get dimensions of text pointing to NULL!");
  }

  if (!font_name) {
    return error(result_text_dimensions, "Cannot get dimensions of text, with font pointing to NULL");
  }
  ...
  text_dimensions dimensions = {.w = text_extents.width, .h = font_extents.height};
...
result_void sdl2_cairo_backend_process_command(const command* cmd) {
  ...
  if (cmd->type == RENDER_RECT) {
    cairo_rectangle(cairo, cmd->bounding_rect.x, cmd->bounding_rect.y, cmd->bounding_rect.w, cmd->bounding_rect.h);
    cairo_set_source_rgba(cairo, cmd->rect_color.r, cmd->rect_color.g, cmd->rect_color.b, cmd->rect_color.a);
    cairo_fill(cairo);
  }
  else {
    cairo_set_source_rgba(cairo, cmd->text_color.r, cmd->text_color.g, cmd->text_color.b, cmd->text_color.a);
...
result_void init_sdl2() {
  ...
  window = SDL_CreateWindow("SDL2 + Cairo Backend", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1080, 720, SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN);
  if (!window) {
...
result_void init_cairo() {
  ...
  if (!cairo_surface) {
    return error(result_void, "Error while creating cairo surface for SDL2 window!");
  }
